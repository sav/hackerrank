...
func (t *BinaryTree) TopView() []int64 {
	var height = t.Height()
	var levels = make([]int64, height*2+1)
	t.BFS(func(node *BinaryTree, level int) bool {
		if levels[height+level] == 0 {
			levels[height+level] = node.Data
		}
		return true
	})
	var view = []int64{}
